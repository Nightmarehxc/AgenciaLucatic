...
public class ReservaController {
	...
	@RequestMapping(value = "/detalle-reserva/{id}", method = RequestMethod.GET)
	public String reservaDetail(@PathVariable("id") String id, Model m) {
		ResponseDTO resDTO = reservaService.getById(id);
		m.addAttribute("reserva", resDTO.getReservas());
		m.addAttribute("usuario", resDTO.getUsuarios());
		m.addAttribute("hotel", resDTO.getHoteles());
		m.addAttribute("vuelo", resDTO.getVuelos());
		return "user_detail";
	}
}